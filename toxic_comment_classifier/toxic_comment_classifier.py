...
# Train and calibrate classifiers for each label using Venn-ABERS
calibrators = {}
for label in ['toxic', 'severe_toxic', 'obscene', 'threat', 'insult', 'identity_hate']:
    # Instantiate the base classifier with the best alpha found
    base_classifier = MultinomialNB(alpha=best_alphas[label])

    # Instantiate Venn-ABERS calibrator
    # Using inductive=True and cal_size=0.2 as per example
    # Using random_state for reproducibility, consistent with train_test_split
    ...
    # The calibrator will internally split train_vectors and train_toxic[label]
    calibrators[label] = va_calibrator
    calibrators[label].fit(train_vectors, train_toxic[label])
...
for label in ['toxic', 'severe_toxic', 'obscene', 'threat', 'insult', 'identity_hate']:
    ...
    val_prob_calibrated = calibrators[label].predict_proba(val_vectors)
    # predict returns class labels based on calibrated probabilities
    val_pred_calibrated = calibrators[label].predict(val_vectors)

    accuracy = accuracy_score(val_toxic[label], val_pred_calibrated)
    precision = precision_score(val_toxic[label], val_pred_calibrated)
    print(f"{label}: Accuracy = {accuracy:.4f}, Precision = {precision:.4f}")

    # Print the confusion matrix
    cm = confusion_matrix(val_toxic[label], val_pred_calibrated)
...
# Store calibrated probabilities and class labels from calibrated predictions
test_predictions_proba = {}
test_predictions_labels = {}

for label in ['toxic', 'severe_toxic', 'obscene', 'threat', 'insult', 'identity_hate']:
    test_predictions_proba[label] = calibrators[label].predict_proba(test_vectors)
    test_predictions_labels[label] = calibrators[label].predict(test_vectors)
...
for label in toxicity_columns:
    ...
    # Use the correct variable for predictions from the calibrated model
    predictions = test_predictions_labels[label]
...
for label in toxicity_columns:
    print(f"\n--- {label} ---")
    # Get the log probability of each feature for the current calibrator
    # feature_log_prob_ is shape (n_classes, n_features)
    # We need to find the index for class 1 (positive class)
    class_index = calibrators[label].classes_.tolist().index(1)
    log_prob_for_class = calibrators[label].feature_log_prob_[class_index]
...
for label in ['toxic', 'severe_toxic', 'obscene', 'threat', 'insult', 'identity_hate']:
    submission_df[label] = test_predictions_labels[label]
